/*
* Signed:YechanKim YOUR NAME: YechanKim Section: 02  Student Number:21600120
*
...
int main(int argc, char *argv[]) {
	...
	key = rand() % (list.back()+1);

		cout << "\n\t" << key << " To Be Found.\n";

	// invoke bineary_search to find key in the list.
	int answer =  binary_search(&list[0],key ,N);
 	if(answer >= 0){
		cout <<"\t"<< key << " is @List[" << answer <<"]\n";
	}
	else {
		cout <<"\t"<< key << " is NOT @List[" << (answer)*-1 <<"]\n";
	}


	// print a few data around a possible key index or -index to check your output.
	int extra = 8;						// a magic number
	int mid = abs(answer) - extra/2;

	for(int i =0; i < extra; mid++, i++){
		if(mid >= 0 && mid < N){
			cout<<"\t["<< mid <<"]="<< list[mid] << endl;
		}
	}
