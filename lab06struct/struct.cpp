...
void step3(const int nCars) {
	...
	for (int i = 0; i < nCars; i++) {
		cout << "[" << i + 1 << "/" << nCars << "]\n";
		arr->model= GetString("\tEnter a model: ");
		arr->speed= GetDouble("\tEnter a speed: ");
		arr ++;
	}
	...
	for (int i = 0; i < nCars; i++) {
	// use for loop to print the car list. don't use print_cars().
		cout << "\t(" << i +1 << ") " << arr->model <<" "<<arr->speed <<  endl ;
		arr ++;
	}
	delete[] arr;
}
...
void set_model(string &model) {
	DPRINT(cout << ">>set_model\n";);
	string models[] = {"BENTLEY", "CORVETTE", "MASERATI", "PORSHE",
						"SALEEN", "STINGRAY", "GENESIS"};
	int size = sizeof(models)/sizeof(models[0]);
